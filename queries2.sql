...
SELECT r.name region,
		a.name account,
        o.total_amt_usd/(o.total+0.01) unite_price
FROM region r
JOIN sales_reps s
ON s.region_id = r.id 
JOIN accounts a
...
Your final table should have only 2 columns: account name and the 
...
SELECT DISTINCT w.channel, a.name, a.id
FROM web_events w
JOIN accounts a
